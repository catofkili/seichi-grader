...
// ---------- 模型持久缓存状态 ----------
const MODEL_CACHE = 'seichi-models-v6'; // v6: 模型 URL 从同源改为 GitHub 绝对地址，旧同源缓存键作废
// 运行时实际读取 ISNet 的四个分块（见 ort-env.js），不是同名的整文件。
// 因此离线包也只缓存分块，避免把同一模型下载两遍。
const ISNET_URL = DEVICE.isAppleMobile ? `${MODEL_BASE}/models/isnet-anime-512-fp16.onnx` : `${MODEL_BASE}/models/isnet-anime-fp16.onnx`;
...
(() => {
  ...
  if (installed) return; // 已安装，无需引导
  let deferredPrompt = null;
  window.addEventListener('beforeinstallprompt', (e) => {
    e.preventDefault(); // 拦下浏览器默认横幅，改为在离线包区域按需展示
    deferredPrompt = e;
    btn.hidden = false;
  });
  btn.addEventListener('click', async () => {
    ...
    if (outcome === 'accepted') { btn.hidden = true; tip.hidden = true; }
    deferredPrompt = null;
  });
  window.addEventListener('appinstalled', () => { btn.hidden = true; tip.hidden = true; });
  // iOS Safari 没有安装 API，只能文字引导
  if (DEVICE.isAppleMobile) {
...
const MODEL_DOWNLOAD_PROBES = [
  ...
  ['GitHub · 抠图模型分块', `${MODEL_BASE}/models/isnet-anime-fp16.onnx.part00`],
