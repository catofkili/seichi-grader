...
1. 在 Wi‑Fi 下点击“下载自动抠图离线包”，确认状态显示“已可离线使用”；SAM 兜底包应保持独立、按需下载。
2. 下载中断后恢复网络，确认按钮改为“继续下载”，且只补未完成的文件。
3. 断网刷新，确认基础调色可用；已下载自动抠图包时，AI 抠图也能启动。
4. 确认 `crossOriginIsolated === true`，桌面设备推理线程数不再是 1。
5. 用 48MP JPEG 验证导出尺寸；低内存手机应明确显示降级尺寸，不能静默缩图。
6. 在 iPhone 导入 HEIC，页面应给出 JPEG 转换提示；导入带 EXIF 旋转的 JPEG，预览方向应正确。
7. 清空站点数据，确认预设/缓存的首次使用体验；刷新后确认参数恢复。
